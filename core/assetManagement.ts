...
export class AssetManager {
  ...
  public async getAssetsByChainIdAndWallet(
    baseDto: BaseDto
  ): Promise<MyAssetsDto> {
    const mutex = this.walletMutexService.getMutex(
      baseDto.chainId,
      baseDto.wallet.toLowerCase()
    )
    return await mutex.runExclusive(async () => {
      return this.accountService.getAssetsByChainId(
        baseDto.wallet,
        baseDto.chainId
      )
    })
  ...
  public async syncAssets(baseDto: BaseDto): Promise<void> {
    ...
    await mutex.runExclusive(async () => {
      return this.accountService.syncAssets(
        context,
        baseDto.wallet,
        baseDto.chainId
      )
  ...
  public async syncOneAsset(syncAssetDto: SyncAssetDto): Promise<void> {
    ...
    await mutex.runExclusive(async () => {
      return this.accountService.syncOneAsset(
        context,
        syncAssetDto.wallet,
        syncAssetDto.chainId,
        syncAssetDto.asset
